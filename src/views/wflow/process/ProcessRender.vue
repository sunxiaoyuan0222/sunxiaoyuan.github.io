...
import processApi from '@/api/wflow/process'
...
export default {
    ...
    const getNode = (tasks, processNodes) => {
      tasks.forEach(task => {
        if (Array.isArray(task.options)) {
          ...
          processNodes.push(
              <el-timeline-item icon={task.icon} size="large" class="task">
                <ProcessNodeRender task={task} desc={task.desc} onAddUser={this.addUser} onDelUser={this.delUser}/>
              </el-timeline-item>
          )
        }
      })
  ...
  methods: {
    ...
    userCompare(explain) {
      let userId = explain.id === 'root' ? this.loginUser.id : this.formData[explain.id]
      //分离人和部门
      let users = [], depts = []
      ...
      if (userCompare) {
        ...
        return 'true' === processApi.userInDepts(userId, depts)
      }
    },
