  ...
  <div v-loading="loading" class="preview">
    ...
    <div class="actions" v-if="!instanceData.result">
      <div style="position: relative; width: 100%;">
        ...
        <div v-if="activeTasks.length > 0" class="action-more">
          <el-dropdown>
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item @click.native="handler('recall')">
                <i class="el-icon-s-release"></i>
                退回
              </el-dropdown-item>
              <el-dropdown-item @click.native="handler('afterAdd')">
                <i class="iconfont icon-zhaopinguanli"></i>
                加签
              </el-dropdown-item>
              <el-dropdown-item v-if="loginUser.id == instanceData.staterUser.id" @click.native="handler('cancel')">
        ...
        <div class="cancel" v-else-if="!instanceData.result && (instanceData.staterUser || {}).id == loginUser.id">
          <el-button type="text" @click="handler('cancel')"><i class="el-icon-refresh-left"></i>撤销</el-button>
        </div>
        <div v-if="activeTasks.length > 0" class="ok-refuse">
          <el-button size="mini" v-if="!isInitiator" type="danger" round plain @click="handler('refuse')">拒绝</el-button>
          <el-button size="mini" type="primary" round @click="handler('agree')">{{isInitiator ? '提交':'同意'}}</el-button>
        </div>
      </div>
    </div>
...
import {getFormAndProcessProgress} from '@/api/wflow/processTask'
import {getCustomPrintConfig} from '@/api/wflow/process'
