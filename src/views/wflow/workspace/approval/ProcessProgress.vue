  ...
  <div class="progress">
    ...
    <div class="progress-item end-process">
      <div class="item-user-avatar node-icon">
        <span :class="[status.icon, 'multi-user-avatar']"></span>
      </div>
...
export default {
  ...
  computed:{
    status(){
      switch (this.result){
        case null: return {text: '审批进行中', icon: 'el-icon-more'}
        case 'refuse-end': return {text: '审批被驳回', icon: 'el-icon-error'}
        case 'cancel-end': return {text: '审批被撤销', icon: 'el-icon-circle-close'}
        default: return {text: '审批通过', icon: 'el-icon-success'}
      }
    }
  },
  data() {
  ...
  methods: {
    ...
    getDesc(item){
      if (item.nodeType === 'ROOT'){
        ...
        if (item.user){
          return `${item.user.name}（${this.getAgree(item)}）`
        }
