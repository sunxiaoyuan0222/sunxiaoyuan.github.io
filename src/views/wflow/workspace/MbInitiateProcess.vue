...
<!--    <nav-bar fixed :title="`发起-${form.formName}`" left-text="取消" right-text="提交" left-arrow @click-left="back" @click-right="submitForm"/>-->
    ...
    <div style="padding-bottom: 5rem">
      ...
      <form-render v-if="!loading" v-show="active === 0" mode="MOBILE" class="process-form" ref="form" :forms="forms" v-model="formData"/>
      <process-render :dept-id="userDeptId"  :pc-mode="false" v-model="processUsers" v-if="!loading && userDeptId" style="padding: 10px" v-show="active === 1" :forms="forms" :formData="formData" :process="process"/>
    </div>
