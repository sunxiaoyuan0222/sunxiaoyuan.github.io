<template>
  <div>
    <el-form-item label="结果描述">
      <el-input size="small" v-model="value.placeholder" placeholder="请设置计算结果描述"/>
    </el-form-item>
    ...
    <w-dialog title="配置计算公式" width="560px" v-model="visible" @ok="validExplain">
      <div slot="title">
      ...
      <el-alert style="margin-bottom: 10px" v-if="showError" type="error" :description="`😢 计算公式/函数设置有误，无法完成计算，${errorInfo}`" :closable="false"/>
  ...
  methods: {
    ...
    modeChange(){
      ...
      if (this.value.isPlus){
        this.jsCodeTemp = this.jsCodeTemp.trim().length > 0 ? this.jsCodeTemp : this.defaultJs
        this.$nextTick(() => {
