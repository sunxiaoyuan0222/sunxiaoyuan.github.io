  ...
  <div>
    ...
    <div v-else-if="mode === 'PC' && !readonly" v-loading="loading">
      <el-upload :file-list="fileList" :action="uploadUrl" :limit="maxNumber" with-credentials
        ...
        <el-button size="small" icon="el-icon-paperclip" round>选择文件1</el-button>
        <ellipsis :row="1" :content="placeholder + sizeTip" hoverTip slot="tip" class="el-upload__tip"/>
      </el-upload>
    </div>
    <div v-else-if="mode === 'MOBILE' && !readonly">
      <uploader v-model="fileList" :accept="String(fileTypes)" 
      :multiple="maxNumber > 1" :max-count="maxNumber > 0 ? maxNumber : 99"
...
export default {
  ...
  methods: {
    ...
    removeFile(fileId) {
      axios.delete(`${this.BASE_URL}/wflow/res/${fileId}`).then(rsp => {
