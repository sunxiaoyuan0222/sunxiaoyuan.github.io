<template>
  <div class="from-panel w-page-body" ref="panel" v-loading="loading">
...
import modelGroupApi from '@/api/wflow/modelGroup'
...
export default {
  ...
  methods: {
    ...
    newProcess(groupId) {
      ...
      this.$tab.openPage('流程设计', `/wflow/design?&groupId=${groupId}`, '_blank');
    },
    groupSort(group) {
    ...
    editModel(item, group) {
      this.$tab.openPage('流程设计', `/wflow/design?code=${item.formId}&groupId=${item.groupId}`, '_blank')
    },
    stopModel(item) {
